...
export async function createTeamMember(data: any): Promise<{ id: number }> {
  const { name, position, bio, shortDesc, email, linkedinUrl, twitterUrl, websiteUrl, resumeUrl } = data;
  if (!name || !position)
    throw Object.assign(new Error("Name and position are required"), { status: 400 });

  const [result] = await db.query<any>(
    `INSERT INTO team_members
     (name, position, bio, short_desc, email, linkedin_url, twitter_url, website_url, resume_url, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
    [name, position, bio || null, shortDesc || null, email || null,
     linkedinUrl || null, twitterUrl || null, websiteUrl || null, resumeUrl || null]
  );
