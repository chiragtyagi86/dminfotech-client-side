...
function parseContent(raw) {
  if (!raw) return { tag: "", tagline: "", features: [] };
      ...
      <Container>
        ...
        <div className="svcgrid">
          {displayServices.map((svc) => (
            <Link key={svc.slug} to={`/services/${svc.slug}`} className="svcgrid-card">
              ...
              {svc.tagline ? <p className="svcgrid-tagline">{svc.tagline}</p> : null}
              ...
              {Array.isArray(svc.features) && svc.features.length > 0 && (
                <div className="svcgrid-features">
                  {svc.features.map((f) => (
                    <span key={f} className="svcgrid-feature">
                      {f}
                    </span>
                  ))}
                </div>
