...
function ResourcesDrawer({ pages = [], isOpen, onClose }) {
  ...
  const filtered = query.trim()
    ? safePages.filter((p) =>
        String(p?.label || "").toLowerCase().includes(query.toLowerCase())
      )
    : safePages;
  ...
  return (
    <>
      <style>{`
        ...
        .res-list-wrap::-webkit-scrollbar {
          width: 4px;
        }
        .res-list-wrap::-webkit-scrollbar-thumb {
        ...
        <div className="res-drawer">
          <div className="res-drawer-header">
            ...
            <div className="res-search-wrap">
              ...
              <input
                ...
                type="text"
                placeholder={`Search ${safePages.length} resources…`}
                value={query}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((page, index) => (
                <Link
                  ...
                  onClick={onClose}
                >
                  <span>{page?.label || "Untitled Page"}</span>
                  <span className="res-item-arrow">
                    <ArrowRight />
                  </span>
                </Link>
              ))
          ...
          <div className="res-drawer-footer">
            <p className="res-result-count">
              {filtered.length === safePages.length
                ? `Showing all ${safePages.length} resources`
                : `${filtered.length} of ${safePages.length} matched`}
            </p>
          </div>
        </div>
      </div>
...
export default function Footer() {
  ...
  const cmsPages = Array.isArray(config.cmsPages) ? config.cmsPages : [];
  ...
  const overflowCount = Math.max(cmsPages.length - PREVIEW_COUNT, 0);

  const openDrawer = useCallback(() => setDrawerOpen(true), []);
  const closeDrawer = useCallback(() => setDrawerOpen(false), []);

  const companyName = config.name || "Dhanamitra Infotech LLP";
  const footerDescription = config.footerDescription || "";
  const siteTagline = config.siteTagline || "";
  ...
  return (
    <footer className="footer-root">
      <style>{`
        ...
        .footer-root {
          background: var(--color-primary, #0f1117);
          position: relative;
          overflow: hidden;
        }
        ...
        <div className="footer-inner">
          <div className="footer-grid">
            <div>
              ...
              {footerDescription && (
                <p className="footer-brand-desc">{footerDescription}</p>
              )}
              ...
              <div className="footer-socials">
                {config.facebookUrl && (
                  <a href={config.facebookUrl} className="footer-social-link" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                    <FacebookIcon />
                  </a>
                )}

                {config.linkedinUrl && (
                  <a href={config.linkedinUrl} className="footer-social-link" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
                    <LinkedinIcon />
                  </a>
                )}

                {config.instagramUrl && (
                  <a href={config.instagramUrl} className="footer-social-link" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                    <InstagramIcon />
                  </a>
                )}

                {config.twitterUrl && (
                  <a href={config.twitterUrl} className="footer-social-link" target="_blank" rel="noopener noreferrer" aria-label="X / Twitter">
                    <TwitterIcon />
                  </a>
                )}

                {config.youtubeUrl && (
                  <a href={config.youtubeUrl} className="footer-social-link" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
                    <YoutubeIcon />
                  </a>
                )}
              </div>
            ...
            <div>
              ...
              <div className="footer-links">
                {navLinks.map((link, index) => (
                  <Link key={link?.href || index} to={link?.href || "#"} className="footer-link">
              ...
              <div>
                ...
                <div className="footer-resources-preview">
                  {previewPages.map((page, index) => (
                    <Link key={page?.href || index} to={page?.href || "#"} className="footer-link">
                      <span>{page?.label || "Untitled Page"}</span>
                      <span className="footer-link-arrow">
                        <ArrowRight />
                      </span>
                ...
                {cmsPages.length > PREVIEW_COUNT && (
                  ...
                  >
                    <span className="footer-resources-trigger-icon">
                      <GridIcon />
                    </span>
            ...
            <div>
              ...
              {config.address && (
                <div className="footer-contact-item">
                  ...
                  <p className="footer-contact-text">{config.address}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="footer-bottom">
            ...
            {siteTagline && <span className="footer-tagline">{siteTagline}</span>}
