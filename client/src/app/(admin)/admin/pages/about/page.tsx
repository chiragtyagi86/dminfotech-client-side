...
// Sections map to: AboutHero, AboutIntro, AboutMissionVision, AboutTrust, AboutPhilosophy
// CMS NOTE: Replace defaults with DB fetch. On save, write to DB/CMS.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function AboutPageEditor() {
  ...
  async function handleSave() {
    setSaving(true);
    // CMS NOTE: POST /api/admin/pages/about with content
  ...
  return (
    <div className="ape-root">
      <style>{`
        ...
        .ape-tab.active { color: var(--color-primary, #3a405a); border-bottom-color: var(--color-primary, #3a405a); font-weight: 500; }
        ...
        .ape-section-label {
          ...
          letter-spacing: 0.20em; text-transform: uppercase; color: rgba(104,80,68,0.35);
          padding-bottom: 8px; border-bottom: 1px solid rgba(104,80,68,0.07); margin-bottom: -6px;
        }
      ...
      <div className="ape-topbar">
        <Link href="/admin/pages" className="ape-back">← Back</Link>
              ...
              {([1, 2, 3] as const).map((n) => (
                <div key={n} className="ape-badge-card">
                  ...
                  <div className="ape-field">
                    ...
                    <input className="ape-input"
                      value={content[`trustBadge${n}` as keyof ContentType]}
                      onChange={(e) => set(`trustBadge${n}` as keyof ContentType, e.target.value)} />
                  </div>
                  <div className="ape-field">
                    <label className="ape-label">Subtitle</label>
                    <input className="ape-input"
                      value={content[`trustBadge${n}Sub` as keyof ContentType]}
                      onChange={(e) => set(`trustBadge${n}Sub` as keyof ContentType, e.target.value)} />
                  </div>
                </div>
              ))}
            </div>
