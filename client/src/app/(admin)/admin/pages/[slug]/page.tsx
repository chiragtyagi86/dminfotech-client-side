...
interface PageSection {
  id: string;
  title: string;
  content: string;      // text content (for text + two-column left)
  imageUrl?: string;    // image URL (for image type + two-column right)
...
export default function PageEditor({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function fetchPage() {
    try {
      ...
      if (!pageData.content || Object.keys(pageData.content).length === 0) {
        ...
        // Migrate legacy two-column sections where imageUrl was stored in content
        const sections = (pageData.content.sections || []).map((s: PageSection) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/pages/${slug}`, {
        ...
        body: JSON.stringify({ title: page.title, description: page.description, content }),
      });
      const json = await res.json();
      if (!res.ok) { alert(json.message || "Failed to save page"); return; }
      setSaved(true);
  ...
  if (loading) return <div style={{ padding: "40px", textAlign: "center" }}>Loading page...</div>;
  if (error) return <div style={{ padding: "40px", textAlign: "center", color: "#c0392b" }}>{error}<br /><Link href="/admin/pages">← Back</Link></div>;
  ...
  return (
    <div className="pe-root">
      <style>{`
        ...
        .pe-heading { font-family: 'Cormorant Garamond', serif; font-size: 28px; font-weight: 400; color: var(--color-primary, #3a405a); margin: 0; flex: 1; }
        ...
        .pe-tab.active { color: var(--color-primary, #3a405a); border-bottom-color: var(--color-primary, #3a405a); font-weight: 500; }
        ...
        .pe-input, .pe-textarea { width: 100%; padding: 12px 13px; border-radius: 9px; border: 1px solid rgba(104,80,68,0.14); background: #fdfaf8; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 300; color: var(--color-primary, #3a405a); outline: none; box-sizing: border-box; }
        ...
        .pe-section-label { font-size: 9.5px; font-weight: 500; letter-spacing: 0.20em; text-transform: uppercase; color: rgba(104,80,68,0.35); padding-bottom: 8px; border-bottom: 1px solid rgba(104,80,68,0.07); margin: 12px 0 -6px; }
        .pe-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        @media (max-width: 640px) { .pe-row { grid-template-columns: 1fr; } }
        .pe-char-counter { font-size: 10.5px; font-weight: 300; text-align: right; margin-top: -10px; }
        ...
        .pe-preview-headline { font-family: 'Cormorant Garamond', serif; font-size: 32px; font-weight: 300; color: var(--color-primary, #3a405a); margin: 0 0 12px; }
        ...
        .pe-section-title-input { flex: 1; min-width: 120px; padding: 8px 10px; border-radius: 6px; border: 1px solid rgba(104,80,68,0.14); background: white; font-family: 'DM Sans', sans-serif; font-size: 13px; color: var(--color-primary, #3a405a); outline: none; }
        .pe-section-title-input:focus { border-color: rgba(153,178,221,0.55); }
        .pe-type-select { padding: 7px 12px; border-radius: 6px; border: 1px solid rgba(153,178,221,0.30); background: rgba(153,178,221,0.10); color: var(--color-primary, #3a405a); font-family: 'DM Sans', sans-serif; font-size: 11px; cursor: pointer; outline: none; }
        .pe-section-remove { padding: 6px 12px; border-radius: 6px; border: none; background: rgba(192,57,43,0.10); color: #c0392b; font-family: 'DM Sans', sans-serif; font-size: 11px; cursor: pointer; white-space: nowrap; }
        ...
        .pe-add-section { padding: 10px 20px; border-radius: 9px; border: 1px solid rgba(153,178,221,0.35); background: rgba(153,178,221,0.10); color: var(--color-primary, #3a405a); font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; cursor: pointer; text-transform: uppercase; letter-spacing: 0.08em; }
        ...
        .pe-save-btn { padding: 10px 24px; border-radius: 9px; border: none; background: var(--color-primary, #3a405a); color: #f9dec9; font-family: 'DM Sans', sans-serif; font-size: 12.5px; font-weight: 500; cursor: pointer; text-transform: uppercase; display: flex; align-items: center; gap: 8px; }
      ...
      <div className="pe-topbar">
        <Link href="/admin/pages" className="pe-back">← Back</Link>
      ...
      <div className="pe-card">
        <div className="pe-tabs">
          {(["hero", "sections", "seo"] as const).map((t) => (
                    ...
                    {section.type === "image" && (
                      <div className="pe-field">
                        ...
                        <input type="text" className="pe-input"
                          value={section.content}
                          placeholder="https://images.pexels.com/... or /images/photo.jpg"
                          onChange={(e) => updateSection(section.id, "content", e.target.value)} />
                        {section.content
                      ...
                      <div className="pe-two-col-grid">
                        <div className="pe-col-panel">
                          ...
                          <textarea className="pe-textarea" style={{ minHeight: "140px", background: "white" }}
                            value={section.content}
                            placeholder="Enter the text for the left column..."
                            onChange={(e) => updateSection(section.id, "content", e.target.value)} />
                        </div>
                        <div className="pe-col-panel">
        ...
        {activeTab === "seo" && (
          <div className="pe-body">
            ...
            <div style={{ padding: "16px", background: "rgba(104,80,68,0.02)", borderRadius: "10px", border: "1px solid rgba(104,80,68,0.08)" }}>
              <p style={{ fontSize: "9.5px", fontWeight: 500, letterSpacing: "0.16em", textTransform: "uppercase", color: "rgba(104,80,68,0.38)", marginBottom: "10px" }}>Search Preview</p>
              <p style={{ fontSize: 18, color: "#1a0dab", marginBottom: 3 }}>{content.seoTitle || page.title}</p>
              <p style={{ fontSize: 12, color: "#006621", marginBottom: 4 }}>dhanamitra.com/{page.slug}</p>
              <p style={{ fontSize: 13, color: "#545454", lineHeight: 1.5 }}>{content.seoDescription || page.description}</p>
            </div>
          </div>
        )}
      </div>
