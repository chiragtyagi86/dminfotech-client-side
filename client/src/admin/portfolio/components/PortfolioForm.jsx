...
export default function PortfolioForm({ initialData, onSubmit, isSaving }) {
  ...
  const [imgPreview, setImgPreview] = useState(
    getImageUrl(initialData?.image ?? initialData?.cover_image ?? initialData?.coverImage ?? "")
  );
  ...
  useEffect(() => {
    ...
    setForm(next);
    setImgPreview(getImageUrl(next.coverImage || ""));
  }, [initialData]);
  ...
  async function handleImg(e) {
    ...
    try {
      ...
      const res = await adminApi.uploadPortfolioImage(fd);
      const imagePath = res?.image || res?.data?.image || "";
      ...
      setImgPreview(getImageUrl(imagePath));
    } catch (err) {
      console.error(err);
      alert(err?.message || "Image upload failed.");
      set("coverImage", "");
      setImgPreview(getImageUrl(form.coverImage || ""));
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div
                style={{
                  ...
                  height: 22,
                  borderRadius: 11,
                  background: form.featured ? "#3a405a" : "rgba(104,80,68,0.15)",
                  position: "relative",
                  cursor: "pointer",
                  transition: "background 0.2s",
                }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div
                style={{
                  ...
                  height: 22,
                  borderRadius: 11,
                  background: form.caseStudyEnabled ? "#3a405a" : "rgba(104,80,68,0.15)",
                  position: "relative",
                  cursor: "pointer",
                  transition: "background 0.2s",
                }}
              ...
              >
                <div
                  style={{
                    position: "absolute",
                    top: 3,
                    left: form.caseStudyEnabled ? 21 : 3,
                    width: 16,
                    height: 16,
                    borderRadius: "50%",
                    background: "#fff",
      ...
      >
        <button type="submit" className="pf-save" disabled={isSaving || uploadingImage}>
          {uploadingImage ? "Uploading Image..." : isSaving ? "Saving…" : isEdit ? "Update Project" : "Create Project"}
        </button>
      </div>
    </form>
  );
