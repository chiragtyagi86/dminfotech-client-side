// pages/team/TeamMembersList.jsx
// Team members list with search, edit, delete actions
  ...
  return (
    <div className="team-members-list">
      <style>{`
        ...
        .team-members-list { display: flex; flex-direction: column; gap: 24px; }
        ...
        .list-search {
          ...
          color: #3a405a; outline: none; min-width: 250px; transition: all 0.2s ease;
        }
        .list-search:focus {
        ...
        .member-card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(58,64,90,0.09); }
        ...
        .card-name { font-family: 'Cormorant Garamond', serif; font-size: 20px; font-weight: 600; color: #3a405a; margin: 0; }
        .card-position { font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; color: rgba(153,178,221,0.80); text-transform: uppercase; letter-spacing: 0.08em; margin: 0; }
        .card-desc { font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 300; color: rgba(104,80,68,0.55); line-height: 1.5; margin: 0; font-style: italic; }
        .card-bio { font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 300; color: rgba(104,80,68,0.55); line-height: 1.5; margin: 4px 0 0; flex: 1; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .card-contact { font-family: 'DM Sans', sans-serif; font-size: 11px; font-weight: 300; color: rgba(104,80,68,0.45); margin-top: 4px; }

        .card-footer { display: flex; gap: 8px; padding: 12px 20px; border-top: 1px solid rgba(104,80,68,0.07); background: rgba(104,80,68,0.02); }
      ...
      <div className="list-header">
        ...
        <div className="list-controls">
          <input type="text" className="list-search" placeholder="Search team members..."
            value={search} onChange={(e) => onSearchChange(e.target.value)} />
          <button className="list-btn" onClick={onAddNew}>+ Add Member</button>
        </div>
      </div>
...
<img
  src={`${import.meta.env.VITE_API_URL}${member.photoUrl || member.photo_url}`}
  alt={member.name}
/>                ) : "👤"}
              </div>
              ...
              <div className="card-body">
                ...
                {(member.shortDesc || member.short_desc) && (
                  <p className="card-desc">{member.shortDesc || member.short_desc}</p>
                )}
                {member.bio && <p className="card-bio">{member.bio}</p>}
                {member.email && <p className="card-contact">📧 {member.email}</p>}
                {member.phone && <p className="card-contact">📞 {member.phone}</p>}
              </div>

              <div className="card-footer">
                <button className="card-btn card-btn-edit" onClick={() => onEdit(member)}>Edit</button>
                <button className="card-btn card-btn-delete" onClick={() => onDelete(member.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
