...
const STATUS_BG = {
contacted: "rgba(153,178,221,0.15)",
  closed: "rgba(249,222,201,0.40)",
  new: "rgba(39,174,96,0.10)",
    ...
    return leads.map((lead) => {
      ...
      return (
        <tr key={lead.id}>
          ...
          <td style={{ color: "rgba(104,80,68,0.45)", fontSize: 12 }}>
            {formatDate(lead.submittedAt || lead.created_at)}
          </td>
          <td>
            <span
              className="ld-badge"
        ...
        <div className="ld-overlay" onClick={() => setViewing(null)}>
          <div className="ld-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {[
              ...
              ["Date", formatDate(viewing.submittedAt || viewing.created_at)],
            ]
              .filter(([, v]) => v)
              .map(([label, val]) => (
