...
export default function PortfolioDetailPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useApi(() => api.getPortfolioItem(slug), [slug]);

  const { data: allItems } = useApi(api.getPortfolioItems);

  if (loading) return <Spinner />;
  if (error || !item) return <Navigate to="/portfolio" replace />;
  ...
  const longDesc = c.longDesc || c.description || item.short_desc || "";
  const year = c.year || String(new Date().getFullYear());
  const accent = c.accent || "rgba(153,178,221,0.30)";
  const projectLink = c.projectLink || item.project_url || "";
  ...
  return (
    <>
    <Seo
      title={`${item.title} | Portfolio — Dhanamitra Infotech LLP`}
      description={item.short_desc || c.longDesc || item.description}
    ...
    />
      ...
      <main>
        <section className="ps-hero">
          ...
          >
            {item.image && (
              <img src={`${import.meta.env.VITE_API_URL}${item.image}`} alt={item.title} className="ps-hero-img" />
            )}
          ...
          <Container>
            <div className="ps-hero-inner">
              <Link to="/portfolio" className="ps-back">
                ← Back to Portfolio
              </Link>

              <div className="ps-tags">
                {item.category && <span className="ps-tag">{item.category}</span>}
              ...
              <div className="ps-main">
                ...
                {gallery.length > 0 && (
                  <div className="ps-gallery">
                    <h2 className="ps-section-label" style={{ marginBottom: 20 }}>
                      Gallery
                    </h2>
                    <div className="ps-gallery-grid">
                      {gallery.map((src, i) => (
                        <div key={i} className="ps-gallery-item">
                          <img src={`${import.meta.env.VITE_API_URL}${src}`} alt={`${item.title} — ${i + 1}`} />
                        </div>
                      ))}
                    </div>
                  </div>
                          ...
                          !p.image
                            ? {
                                background: `linear-gradient(145deg, ${
                                  pc.accent || "rgba(153,178,221,0.30)"
