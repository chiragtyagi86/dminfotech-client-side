...
function normalizeMember(item) {
  ...
  return {
    ...
    signature: getMediaUrl(item?.signature || null),
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .tmdet-signature-wrap { margin-top: 28px; }
        .tmdet-signature-label { font-family: 'DM Sans', sans-serif; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--color-text-soft); margin-bottom: 8px; }
        .tmdet-signature-img { max-width: 180px; height: auto; display: block; object-fit: contain; }
      `}</style>
            ...
            <div className="tmdet-hero-layout">
              <div>
                ...
                <div className="tmdet-socials">
                  {member.email && (
                    <a href={`mailto:${member.email}`} className="tmdet-social">
                      ✉ Email
                    </a>
                  )}
                  ...
                  {member.resumeUrl && (
                    <a href={member.resumeUrl} target="_blank" rel="noopener noreferrer" className="tmdet-social">
                      ⤓ Resume
                    </a>
                  )}
                </div>
                ...
                {member.signature && (
                  <div className="tmdet-signature-wrap">
                    <div className="tmdet-signature-label">Signature</div>
                    <img
                      src={`${import.meta.env.VITE_API_URL}${member.signature}`}
                      alt={`${member.name} signature`}
                      className="tmdet-signature-img"
                    />
                  </div>
                )}
              </div>
