...
export default function CareersPage() {
  const { data: jobs, loading } = useApi(api.getJobs);
  const allJobs = jobs || [];
  ...
  return (
    ...
    <Seo
      ...
      keywords={[
        ...
        "careers",
        "job openings",
        "digital solutions company jobs",
        "software development careers",
        "IT placements jobs",
        "stock market training careers",
        "ISO certified company jobs",
    ...
    <main>
      <style>{`
        ...
        .car-hero-grid {
          ...
          -webkit-mask-image: radial-gradient(ellipse 60% 80% at 10% 50%, black, transparent);
        ...
        <Container>
          <div className="car-hero-inner">
            ...
            <p className="car-hero-sub">
              We're a small team that moves fast, works on real problems and cares
              about the quality of everything we ship. If that sounds like you,
              read on.
            </p>
          </div>
        </Container>
      ...
      <section className="car-jobs-section">
        <Container>
          <SectionHeading
            ...
            description={
              allJobs.length > 0
                ? `${allJobs.length} open position${allJobs.length > 1 ? "s" : ""} — click any role to read more and apply.`
          ...
          {loading ? (
            ...
            <div className="car-empty">
              ...
              <p className="car-empty-text">
                We'll post new roles here as we grow. Follow us on LinkedIn to stay
                updated.
              </p>
            </div>
          ) : (
              ...
              {allJobs.map((job) => (
                ...
                >
                  <div className="car-card-left">
                    {job.department && (
                      <p className="car-card-dept">{job.department}</p>
                    )}
                    ...
                    <div className="car-tags">
                      ...
                      {(job.location_type || job.locationType) && (
                        <span className="car-tag">
                          <span className="car-tag-dot" />
                          {locationTypeLabel(job.location_type || job.locationType)}
                        </span>
                      )}
                      {(job.job_type || job.jobType) && (
                        <span className="car-tag">
                          <span className="car-tag-dot" />
                          {jobTypeLabel(job.job_type || job.jobType)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="car-card-right">
                    <span className="car-salary">
                      {salaryDisplay(
                        job.salary_min || job.salaryMin,
                        job.salary_max || job.salaryMax,
                        job.salary_currency || job.salaryCurrency
                      )}
                    </span>
