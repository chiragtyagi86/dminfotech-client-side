...
function normalizeFormData(initialData) {
  ...
  // tags can be an array or comma string from DB
  ...
  return {
    ...
    tags,
    // handle both camelCase and snake_case image field names
    featuredImage:   initialData.featuredImage   ?? initialData.featured_image ?? initialData.image ?? "",
    published:
      typeof initialData.published === "boolean"
        ? initialData.published
        : initialData.status === "published" || initialData.status === "active",
    // handle both camelCase and snake_case meta fields
...
export default function BlogForm({ initialData, onSubmit, isSaving }) {
  ...
  function handleImage(e) {
    ...
    // keep existing image path until server confirms upload
    // _imageFile signals to parent that a new file was chosen
  }
  ...
  function handleSubmit(e) {
    ...
    const payload = {
      ...
      tags:            tagsArray,
      featured_image:  form.featuredImage,   // snake_case for backend
    ...
    // attach file separately so parent can decide whether to use multipart
