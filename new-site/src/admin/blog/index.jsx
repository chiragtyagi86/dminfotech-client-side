...
function normalizePost(post) {
  return {
    ...
    createdAt: post.createdAt ?? post.created_at ?? post.publishedAt ?? post.published_at ?? null,
  };
}
  ...
  return (
    <div className="bl-root">
      ...
      <div className="bl-topbar">
        ...
        <Link to="/admin/blog/new" className="bl-new-btn">+ New Post</Link>
      </div>
      ...
      <div className="bl-table">
        <div className="bl-table-wrap">
          <table>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} className="bl-empty">Loading…</td></tr>
              ) : posts.length === 0 ? (
                <tr><td colSpan={5} className="bl-empty">No posts found.</td></tr>
              ) : posts.map((post) => (
                <tr key={post.slug ?? post.id}>
                  <td style={{ fontWeight: 400 }}>{post.title}</td>
                  <td style={{ color: "rgba(104,80,68,0.55)" }}>{post.category || "—"}</td>
                  <td>
                  ...
                  <td>
                    <div className="bl-actions">
                      <Link to={`/admin/blog/${post.slug}`} className="bl-edit-btn">Edit</Link>
                      <button
